    ...
    for (const templateId of templateIds) {
      ...
      const toUpdate = allFields
        ...
        .filter((f) => {
          ...
          // needs update if: not square, OR both square but below minimum 1
          return Math.abs(w - h) > 0.001 || h < 1;
        });
      ...
      const updatePayload = {
        ...
        fields: toUpdate.map((f) => {
          ...
          const newSize = Math.max(1, oldH); // Documenso minimum is 1
      ...
      if (!updateRes.ok) {
        ...
        console.error(`[bulk-square-fields] Template ${templateId} update failed: ${updateRes.status}`, errBody);
        results.errors.push(`Template ${templateId} update failed: ${updateRes.status} — ${errBody}`);
        continue;
      }
      ...
      const summary = updatePayload.fields.map((f) => {
        ...
        return {
          ...
          page: orig?.page ?? "",
          widthBefore: parseFloat(orig.width),
          widthAfter: f.width,
        };
      });

      results.updated.push(...summary);
    }
